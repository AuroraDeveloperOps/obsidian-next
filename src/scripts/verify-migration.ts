...
async function verify() {
	...
	const workingSet = database
		.prepare('SELECT * FROM working_set WHERE session_id = ?')
		.all(sessionId);
	if (workingSet.length !== 2)
		throw new Error(
			`Working set count mismatch. Expected 2, got ${workingSet.length}`
		);
	...
	if (!taskRow || taskRow.title !== 'Migration Verification')
	...
	const usageRow = database
		.prepare('SELECT * FROM usage_stats WHERE session_id = ?')
		.get(sessionId) as any;
	if (!usageRow || usageRow.input_tokens !== 100)
	...
	if (restoredCtx.session_id !== sessionId)
		throw new Error('Restored session ID mismatch');
	if (restoredCtx.working_set.length !== 2)
		throw new Error('Restored working set mismatch');

	// Verify Task Restoration
	const currentTask = tasks.get();
	if (!currentTask || currentTask.title !== 'Migration Verification')
		throw new Error('Restored task mismatch');
