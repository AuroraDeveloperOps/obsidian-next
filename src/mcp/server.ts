...
export function createMcpServer() {
	...
	server.registerTool(
		'bash',
		{
			...
			description:
				'Execute a shell command in the workspace. Use for git, npm, and other CLI operations.',
			inputSchema: {
		...
		async ({ command, timeout }) => {
			...
			if (blocked.some((p) => command.includes(p))) {
				return {
					content: [
						{ type: 'text', text: 'Error: Blocked dangerous command pattern' }
					],
					isError: true
				};
			}
					...
					content: [
						{
							type: 'text',
							text: truncateOutput(
								stdout || stderr || 'Command executed successfully'
							)
						}
					]
				};
	...
	server.registerTool(
		...
		async ({ path: filePath, offset, limit }) => {
			try {
				...
				const endLine = limit
					? startLine + limit
					: Math.min(startLine + MAX_FILE_READ_LINES, lines.length);
			...
			try {
				...
				try {
					...
					return {
						content: [
							{
								type: 'text',
								text: 'Error: File already exists. Use edit tool instead.'
							}
						],
						isError: true
					};
				...
				return {
					content: [
						{
							type: 'text',
							text: `Created: ${filePath} (${content.length} bytes)`
						}
					]
				};
			} catch (error: any) {
		...
		async ({ path: filePath, search, replace }) => {
			try {
				...
				if (!original.includes(search)) {
					return {
						content: [
							{ type: 'text', text: 'Error: Search string not found in file' }
						],
						isError: true
					};
				}
	...
	server.registerTool(
		...
		async ({ pattern, path: searchPath, limit }) => {
			try {
				...
				if (results.length === 0) {
					return {
						content: [{ type: 'text', text: `No matches for: ${pattern}` }]
					};
				}
	...
	server.registerTool(
		...
		async ({ pattern, path: basePath }) => {
			try {
				...
				if (results.length === 0) {
					return {
						content: [{ type: 'text', text: `No files matching: ${pattern}` }]
					};
				}
		...
		async ({ url }) => {
			try {
				...
				if (blocked.some((d) => urlObj.hostname.includes(d))) {
					return {
						content: [
							{ type: 'text', text: 'Error: Cannot fetch from local addresses' }
						],
						isError: true
					};
				}
	...
	server.registerTool(
		'computer',
		{
			title: 'Computer Use',
			description:
				'Control the computer (screenshot, keyboard, mouse). Available actions: screenshot, key, type, mouse_move, left_click, left_click_drag, right_click, middle_click, double_click, cursor_position.',
			inputSchema: {
				action: z
					.enum([
						'screenshot',
				...
				coordinate: z
					...
					.length(2)
					.optional()
					.describe('Coordinates [x, y] for mouse actions'),
				text: z.string().optional().describe('Text to type or key to press'),
				start_coordinate: z
					.array(z.number())
					.length(2)
					.optional()
					.describe('Start coordinates [x, y] for drag')
			}
		},
		async ({ action, coordinate, text, start_coordinate }) => {
			try {
				...
				switch (action) {
					...
					default:
						throw new Error(
							`Action ${action} not implemented or supported via MCP yet`
						);
				}
