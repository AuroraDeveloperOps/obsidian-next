...
class KeyManager {
	...
	async storeKey(
		key: string,
		options: { service?: string; account?: string } = {}
	): Promise<{ success: boolean; backend: StorageBackend; error?: string }> {
		const service = options.service || DEFAULT_SERVICE;
		const account = options.account || DEFAULT_ACCOUNT;
		const cacheKey = this.getCacheKey(service, account);
	 ...
	 */
	shouldRotate(
		config: KeyConfig = this.keyCache.get(
	...
	getBackend(
		...
		const account = options.account || DEFAULT_ACCOUNT;
		return (
			this.keyCache.get(this.getCacheKey(service, account))?.backend || null
		);
	}

	// ==================== macOS Keychain ====================

	private async loadFromKeychain(
		service: string,
		account: string
	): Promise<string | null> {
		try {
	...
	private async deleteFromKeychain(
		service: string,
		account: string
	): Promise<void> {
		try {
	...
	private async loadFromSecretTool(
		service: string,
		account: string
	): Promise<string | null> {
		try {
	...
	private async storeInSecretTool(
		key: string,
		service: string,
		account: string
	): Promise<{ success: boolean; error?: string }> {
	...
	private async deleteFromSecretTool(
		service: string,
		account: string
	): Promise<void> {
		try {
	...
	private async loadFromEncryptedFile(
		service: string,
		account: string
	): Promise<string | null> {
		try {
		...
		try {
			...
			try {
				...
				try {
					...
					// Legacy was string
					keyMap[this.getCacheKey(DEFAULT_SERVICE, DEFAULT_ACCOUNT)] =
						exDecrypted;
				}
			} catch {
				// No existing file
...
export async function detectEnvFile(
