...
/** DB row types for type-safe queries */
...
class SessionManager {
		...
		try {
			...
			for (const row of rows) {
                ...
                `
					...
					.get(row.id) as CountRow | undefined;
	...
	async delete(sessionId: string): Promise<boolean> {
		try {
			const transaction = db.getDb().transaction(() => {
				db.getDb()
					.prepare(
						'DELETE FROM subtasks WHERE task_id IN (SELECT id FROM tasks WHERE session_id = ?)'
					)
					.run(sessionId);
				db.getDb()
					...
					.run(sessionId);
				db.getDb()
					.prepare('DELETE FROM working_set WHERE session_id = ?')
					.run(sessionId);
				db.getDb()
					.prepare('DELETE FROM usage_stats WHERE session_id = ?')
					.run(sessionId);
				db.getDb()
	...
	async restore(
        ...
        `
			...
			.get(sessionId) as UsageStatsRow | undefined;

		const duration =
			stats?.lastLog && stats?.firstLog ? stats.lastLog - stats.firstLog : 0;
		...
		if (row.llm_history) {
			try {
				const restoredHistory = JSON.parse(row.llm_history) as Parameters<
					typeof llm.restoreHistory
				>[0];
