...
export class MemoryManager {
	...
	async init(): Promise<void> {
		...
		try {
			...
			bus.emitAgent({
				type: 'error',
				message: `[Memory] Embedding pipeline failed to initialize: ${e instanceof Error ? e.message : String(e)}`
			});
		}
	...
	async store(type: MemoType, key: string, content: string): Promise<boolean> {
		...
		try {
			...
			const transaction = db.getDb().transaction(() => {
				...
				// Update vector table
				db.getDb()
					.prepare('DELETE FROM vec_memos WHERE rowid = ?')
					.run(BigInt(memoId));
				db.getDb()
					.prepare(
						`
			...
			bus.emitAgent({
				type: 'error',
				message: `Memory Store Error: ${e instanceof Error ? e.message : String(e)}`
			});
			return false;
		}
	}
		...
		try {
			...
			return {
				...
				last_accessed: row.last_accessed
					? new Date(row.last_accessed * 1000).toISOString()
					: undefined
			};
		} catch (e) {
			bus.emitAgent({
				type: 'error',
				message: `[Memory] Recall failed for key "${key}": ${e instanceof Error ? e.message : String(e)}`
			});
			return null;
		}
	}
	...
	async search(query: string, type?: MemoType): Promise<Memo[]> {
		...
		try {
			...
			bus.emitAgent({
				type: 'thought',
				content: `[Memory] Semantic search failed, falling back to keyword search.`,
				hidden: true
			});
			return this.keywordSearch(query, type);
		}
	...
	async getByType(type: MemoType): Promise<Memo[]> {
		...
		try {
			...
			bus.emitAgent({
				type: 'error',
				message: `[Memory] Failed to get memories by type "${type}": ${e instanceof Error ? e.message : String(e)}`
			});
			return [];
		}
	}
	...
	async forget(key: string): Promise<boolean> {
		...
		try {
			...
			bus.emitAgent({
				type: 'error',
				message: `[Memory] Failed to forget key "${key}": ${e instanceof Error ? e.message : String(e)}`
			});
			return false;
		}
	}
	...
	async clearAll(): Promise<boolean> {
		...
		try {
			...
			bus.emitAgent({
				type: 'error',
				message: `[Memory] Failed to clear all memories: ${e instanceof Error ? e.message : String(e)}`
			});
			return false;
		}
	}
	...
	async exportToMarkdown(): Promise<string> {
		...
		try {
			...
			bus.emitAgent({
				type: 'error',
				message: `[Memory] Export failed: ${error.message}`
			});
	...
	async addRelation(
		...
		try {
			...
			bus.emitAgent({
				type: 'error',
				message: `[Memory] Failed to add relation: ${e instanceof Error ? e.message : String(e)}`
			});
			return false;
		}
	}
	 ...
	 */
	async getRelatedMemos(
		key: string
	): Promise<{ memo: Memo; relation: MemoRelation }[]> {
		await this.init();
	...
	getRelevanceScore(memo: Memo): number {
		const now = Date.now();
		const lastAccessed = memo.last_accessed
			? new Date(memo.last_accessed).getTime()
			: now;
	...
	async searchWithRelations(
		query: string,
		type?: MemoType
	): Promise<{ memo: Memo; score: number; related: Memo[] }[]> {
		await this.init();
