...
export class MigrationManager {
	private migrations: Migration[] = [
		{
			...
			up: (db: Database) => {
				...
				bus.emitAgent({
					type: 'thought',
					content: '[Migration] Updating memos table schema...',
					hidden: true
				});
				...
				// 5. Re-create indexes
				db.exec(
					'CREATE INDEX IF NOT EXISTS idx_memos_type_key ON memos(type, key);'
				);
				db.exec(
				...
				bus.emitAgent({
					type: 'thought',
					content: '[Migration] Memos table updated successfully.',
					hidden: true
				});
			}
		},
		{
			...
			up: (db: Database) => {
				const tableInfo = db
					...
					.all() as any[];
				const hasCacheRead = tableInfo.some(
					(col) => col.name === 'cache_read_tokens'
				);
				const hasCacheCreation = tableInfo.some(
				...
				bus.emitAgent({
					type: 'thought',
					content: '[Migration] Added cache token columns to usage_stats.',
					hidden: true
				});
			}
		},
		{
			...
			up: (db: Database) => {
                ...
                `);
				db.exec(
					'CREATE INDEX IF NOT EXISTS idx_session_permissions ON session_permissions(session_id, permission_type);'
				);

				bus.emitAgent({
					type: 'thought',
					content: '[Migration] Added session_permissions table.',
					hidden: true
				});
			}
		},
		{
			...
			up: (db: Database) => {
                ...
                `);
				db.exec(
					'CREATE INDEX IF NOT EXISTS idx_memo_relations_source ON memo_relations(source_id);'
				);
				db.exec(
					'CREATE INDEX IF NOT EXISTS idx_memo_relations_target ON memo_relations(target_id);'
				);

				// Add temporal decay columns to memos table
				const tableInfo = db.prepare('PRAGMA table_info(memos)').all() as any[];
				const hasAccessCount = tableInfo.some(
					(col) => col.name === 'access_count'
				);
				const hasLastAccessed = tableInfo.some(
					(col) => col.name === 'last_accessed'
				);

				if (!hasAccessCount) {
					db.exec(
						'ALTER TABLE memos ADD COLUMN access_count INTEGER DEFAULT 0'
					);
				}
				if (!hasLastAccessed) {
					...
					db.exec(
						'ALTER TABLE memos ADD COLUMN last_accessed INTEGER DEFAULT NULL'
					);
				}
				...
				bus.emitAgent({
					type: 'thought',
					content:
						'[Migration] Added memory graph support: relations table and temporal decay columns.',
					hidden: true
				});
			}
		}
	...
	async run(db: Database): Promise<void> {
		...
		const applied = db.prepare('SELECT version FROM _migrations').all() as {
			version: number;
		}[];
