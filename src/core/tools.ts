 ...
 *
 * This is for tracking in-session work items (plan steps, subtasks).
 * NOT for scheduled/recurring background jobs -- use schedule_task for those.
 */
...
/**
 * HTTP Request Tool - Full HTTP method support
 */
...
/**
 * Delete Tool - Delete files/directories with safety checks
 */
export const DeleteTool: Tool = {
    ...
    async execute(args: Record<string, any>): Promise<ToolResult> {
        ...
        try {
            ...
            await auditLog.logFileOperation('delete', filePath, false, error.message);
    ...
    async execute(args: Record<string, any>): Promise<ToolResult> {
        ...
        try {
            if (process.platform === 'darwin') {
                ...
                await execAsync(`osascript -e 'display notification "${escapedMessage}" with title "${escapedTitle}" sound name "${sound}"'`).catch(() => {});
...
Provide EITHER cron/interval (recurring) OR at (one-time), plus ability and optional params.`,
    ...
    async execute(args: Record<string, any>): Promise<ToolResult> {
        ...
        if (at) {
            ...
            try {
                const task = await scheduler.scheduleTask(cronExpr, ability, { ...params, __once: true, __target: targetDate.getTime() });
                return {
                    success: true,
