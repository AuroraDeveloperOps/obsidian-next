...
class TaskTracker {
	...
	async load(): Promise<void> {
		try {
			...
			bus.emitAgent({
				type: 'error',
				message: `[Tasks] Failed to load: ${e instanceof Error ? e.message : String(e)}`
			});
			this.task = null;
		}
	...
	async save(): Promise<void> {
		...
		try {
			...
			const updateSubtasks = db.getDb().transaction(() => {
				...
				db.getDb()
					.prepare('DELETE FROM subtasks WHERE task_id = ?')
					.run(this.task!.id);

				const stmt = db
					.getDb()
					.prepare(
						'INSERT INTO subtasks (id, task_id, text, done, position) VALUES (?, ?, ?, ?, ?)'
					);
			...
			bus.emitAgent({
				type: 'error',
				message: `[Tasks] Failed to save: ${e instanceof Error ? e.message : String(e)}`
			});
		}
	}
