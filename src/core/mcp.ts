...
class MCPManager {
	...
	async connect(name: string, serverConfig: ServerConfig) {
		...
		try {
			...
			const sanitizedEnv = serverConfig.env
				? Object.fromEntries(
			...
			if (serverConfig.secureEnv) {
				for (const envName of serverConfig.secureEnv) {
					...
					if (key) {
						...
						console.warn(
							`[MCP] Missing secure key for ${envName} (server: ${name})`
						);
					}
				}
			}
		...
		if (conn) {
			try {
				...
				try {
					...
					if (
						transport._process &&
						typeof transport._process.kill === 'function'
					) {
				...
				if (
					this.config.servers[name] &&
					this.config.servers[name].autoConnect
				) {
	...
	async callTool(serverName: string, toolName: string, args: any) {
		...
		try {
			...
			throw new Error(
				`Tool execution failed on ${serverName}/${toolName}: ${error}`
			);
		}
	}
