...
function getModifier(modifier?: string): string {
  ...
  switch (modifier.toLowerCase()) {
    ...
    case 'alt': return 'alt-';
    case 'super': return 'cmd-'; // 'super' usually maps to 'cmd' on macOS
    default: return '';
  }
}

export async function leftClick(x: number, y: number, modifier?: string): Promise<void> {
...
export async function middleClick(x: number, y: number, modifier?: string): Promise<void> {
  ...
  console.warn('Middle click not directly supported by cliclick. Consider alternative implementation if needed.');
  await shell.execute(`echo "Middle click at ${x},${y} with modifier ${modifier} requested but not fully supported by cliclick."`);
}

export async function doubleClick(x: number, y: number, modifier?: string): Promise<void> {
  const mod = getModifier(modifier);
  await shell.execute(`cliclick ${mod}dc:${x},${y}`);
}

export async function tripleClick(x: number, y: number, modifier?: string): Promise<void> {
  const mod = getModifier(modifier);
...
export async function scroll(x: number, y: number, direction: 'up' | 'down' | 'left' | 'right', amount: number, modifier?: string): Promise<void> {
  let scrollCommand = '';
  const mod = getModifier(modifier);
