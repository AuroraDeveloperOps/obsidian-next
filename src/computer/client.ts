 ...
 */
export function getToolConfig(model: string): {
	...
	try {
		...
		switch (action) {
			...
			case 'left_click': {
				...
				return {
					content: [
						{
							type: 'text',
							text: `Clicked at (${x}, ${y})${modifier ? ` with ${modifier}` : ''}`
						},
						{
							type: 'image',
							source: {
			...
			case 'type': {
				...
				return {
					content: [
						{
							type: 'text',
							text: `Typed: "${input.text.substring(0, 50)}${input.text.length > 50 ? '...' : ''}"`
						},
						{
							type: 'image',
							source: {
			...
			case 'hold_key': {
				...
				return {
					content: [
						{ type: 'text', text: `Held key ${input.key} for ${duration}s` }
					],
					isError: false
				};
			}
		...
		switch (command) {
			...
			case 'str_replace': {
				...
				if (!original.includes(input.old_str)) {
					return {
						content: [
							{ type: 'text', text: `Error: old_str not found in file` }
						],
						isError: true
					};
				}
			...
			case 'insert': {
				...
				return {
					content: [
						{
							type: 'text',
							text: `Inserted text at line ${insertLine} in ${filePath}`
						}
					],
					isError: false
				};
			...
			case 'undo_edit': {
				return {
					content: [
						{ type: 'text', text: 'Undo not implemented - use version control' }
					],
					isError: true
				};
			}
			...
			default:
				return {
					content: [
						{ type: 'text', text: `Unknown editor command: ${command}` }
					],
					isError: true
				};
		}
