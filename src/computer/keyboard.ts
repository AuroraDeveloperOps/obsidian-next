...
async function pressKeyCombination(combo: string): Promise<void> {
  ...
  if (mappedKey) {
    ...
    const escapedKey = mainKey
      ...
      .replace(/"/g, '\\"')
      .replace(/'/g, "\\'");
    await shell.execute(`cliclick ${holdCmds} t:"${escapedKey}" ${releaseCmds}`);
  }
}
...
export async function holdKey(key: string, duration: number): Promise<void> {
  ...
  await shell.execute(`cliclick kd:${cliclickKey} w:${Math.round(durationSec * 1000)} ku:${cliclickKey}`);
}
