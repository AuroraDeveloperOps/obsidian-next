...
function waitForTextInput(
	...
	return new Promise((resolve) => {
		...
		const handler = (event: any) => {
			if (
				event.type === 'text_input_response' &&
...
async function configureMode(): Promise<void> {
	...
	if (selection === 'auto') {
		bus.emitAgent({
			type: 'thought',
			content:
				'[WARN] Auto mode will execute all commands without confirmation!'
		});
	}
...
async function configureSecurity(): Promise<void> {
	...
	if (selection === 'pii') {
		...
		await settings.save({
			security: { ...s.security, piiRedaction: newValue }
		});
		bus.emitAgent({
			type: 'done',
	...
	if (selection === 'audit') {
		...
		await settings.save({
			security: { ...s.security, auditLogging: newValue }
		});
		bus.emitAgent({
			type: 'done',
...
async function configurePermissions(): Promise<void> {
	...
	const question = [content, 'Permission Actions'].join('\n');
	...
	if (selection === 'add-allow') {
		const { value, cancelled } = await waitForTextInput(
			'Enter pattern (tool:command, e.g., bash:git*):'
		);
		if (!cancelled && value) {
			...
			bus.emitAgent({
				type: 'done',
				summary: `Added "${value}" to allow list`
			});
		}
	}
	...
	switch (key) {
		...
		case 'maxTokens':
			...
			if (isNaN(num) || num < 100 || num > 200000) {
				bus.emitAgent({
					type: 'error',
					message: 'maxTokens must be between 100 and 200000'
				});
				return;
			}
		...
		case 'mode':
			if (!['auto', 'plan', 'safe'].includes(value)) {
				bus.emitAgent({
					type: 'error',
					message: 'mode must be: auto, plan, or safe'
				});
				return;
			}
		...
		case 'auditLogging':
			...
			await settings.save({
				security: { ...s2.security, auditLogging: audit }
			});
			break;
...
async function resetConfig(): Promise<void> {
	const confirm = await waitForChoice('Reset all configuration to defaults?', [
...
async function exportConfig(filepath?: string): Promise<void> {
	...
	const outputPath =
		filepath || path.join(os.homedir(), 'obsidian-config-export.json');
