    ...
    private setupListeners() {
        ...
        bus.on('agent', async (event) => {
            if (event.type === 'scheduler_task_completed') {
                await agent.run(`The scheduled task '${event.command}' has completed successfully.`);
            }

            if (event.type === 'scheduler_task_failed') {
                await agent.run(`The scheduled task '${event.command}' failed with error: ${event.error}`);
            }
        })
    }
