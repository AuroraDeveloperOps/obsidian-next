...
export function registerSystemAbilities(sched: Scheduler) {
	...
	sched.registerAbility('system:audit', async () => {
		...
		bus.emitAgent({
			type: 'thought',
			content: '[Audit] Starting proactive security scan...'
		});
		...
		try {
			...
			for (const pattern of sensitivePatterns) {
				try {
					const { stdout } = await execAsync(
						`find ${cfg.workspaceRoot} -name "${pattern}" -not -path "*/node_modules/*"`
					);
			...
			if (found.length > 0) {
				...
				bus.emitAgent({
					type: 'thought',
					content:
						'[Audit] No immediate security risks found in workspace root.'
				});
			}
		} catch (e) {
	...
	sched.registerAbility('system:index', async () => {
		...
		bus.emitAgent({
			type: 'thought',
			content: '[Index] Updating codebase map (MAP.md)...'
		});
		...
		try {
			...
			const { stdout } = await execAsync(
				`find ${cfg.workspaceRoot} -maxdepth 2 -not -path '*/.*'`
			);
			...
			bus.emitAgent({
				type: 'thought',
				content: `[Index] MAP.md updated successfully at ${mapPath}`
			});
		} catch (e) {
		...
		if (process.platform === 'darwin') {
			try {
				...
				if (
					...
					bus.emitAgent({
						type: 'thought',
						content: `[Obsidian] High-priority reminder protocol initiated.`
					});
