...
const colorizeLine = (line: string, isError: boolean): React.ReactNode => {
	...
	// Success indicators
	if (
		displayLine.includes('success') ||
		displayLine.includes('Success') ||
		displayLine.includes('✓') ||
		displayLine.includes('Build success')
	) {
		return <Text color="green">{displayLine}</Text>;
	}
	// Warning indicators
	if (
		displayLine.includes('warning') ||
		displayLine.includes('Warning') ||
		displayLine.includes('WARN')
	) {
		return <Text color="yellow">{displayLine}</Text>;
	}
	// Error indicators
	if (
		displayLine.includes('error') ||
		displayLine.includes('Error') ||
		displayLine.includes('ERR') ||
		displayLine.includes('failed')
	) {
		return <Text color="red">{displayLine}</Text>;
	}
	// File paths
	if (
		displayLine.match(/^[a-zA-Z].*\.(ts|js|tsx|jsx|json|md|css|html)/) ||
		displayLine.includes('dist/') ||
		displayLine.includes('src/')
	) {
		return <Text color="cyan">{displayLine}</Text>;
	}
