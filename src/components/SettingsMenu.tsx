...
export const SettingsMenu: React.FC<SettingsMenuProps> = ({
	...
	const getMenuItems = useCallback((): MenuItem[] => {
		...
		switch (view) {
				...
				return [
					{
						...
						type: 'category',
						description: `Current: ${currentSettings.mode} (Shift+Tab to cycle)`
					},
					{
						key: 'provider',
						label: 'Provider Settings',
						type: 'category',
						description: `Mode: ${(currentConfig as any).provider || 'anthropic'}`
					},
					{
						key: 'models',
						label: 'Model Selection',
						type: 'category',
						description: `Current: ${currentConfig.model.split('-').slice(0, 2).join(' ')}`
					},
					{
						key: 'security',
						label: 'Security',
						type: 'category',
						description: 'PII, audit, sandbox'
					},
					{
						key: 'ui',
						label: 'UI Preferences',
						type: 'category',
						description: 'Syntax, colors'
					},
					{
						key: 'permissions',
						label: 'Permissions',
				...
				return [
					{
						...
						label: 'Auto Mode',
						type: 'select',
						value: currentSettings.mode === 'auto',
						description: 'Execute without confirmation (Use with caution)'
					},
					{
						key: 'plan',
						label: 'Plan Mode',
						type: 'select',
						value: currentSettings.mode === 'plan',
					...
					},
					{
						...
						label: 'Safe Mode',
						type: 'select',
						value: currentSettings.mode === 'safe',
						description: 'Require approval for writes'
					},
					{ key: 'back', label: 'Back', type: 'action' }
				];
			case 'provider':
				const currentProvider = (currentConfig as any).provider || 'anthropic';
				return [
					{
						...
						description: 'API-only, requires API key'
					},
					{
						key: 'provider-ollama',
						label: 'Ollama (Local Models)',
						type: 'select',
						value: currentProvider === 'ollama',
						description: 'Offline, requires Ollama'
					},
					{
						key: 'provider-moe',
						label: 'MoE (Mixture of Experts)',
						type: 'select',
						value: currentProvider === 'moe',
						description: 'Intelligent routing between models'
					},
					{ key: 'back', label: 'Back', type: 'action' }
				];
			case 'models':
				...
				return [
					{
						...
						type: 'select',
						value: currentModel === 'claude-opus-4-6-20260207',
					...
					},
					{
						...
						type: 'select',
						value: currentModel === 'claude-opus-4-5-20251101',
					...
					},
					{
						...
						type: 'select',
						value: currentModel === 'claude-sonnet-4-5-20250929',
					...
					},
					{
						...
						type: 'select',
						value: currentModel === 'claude-haiku-4-5-20251001',
						description: 'Fastest'
					},
					{ key: 'back', label: 'Back', type: 'action' }
				];
				...
				return [
					{
						...
						label: 'PII Redaction',
						type: 'toggle',
						value: currentSettings.security.piiRedaction,
					...
					},
					{
						...
						label: 'Audit Logging',
						type: 'toggle',
						value: currentSettings.security.auditLogging,
					...
					},
					{
						...
						label: 'Key Storage',
						type: 'category',
						description: `Current: ${currentSettings.security.keyBackend}`
					},
					{ key: 'back', label: 'Back', type: 'action' }
				];
			case 'ui':
				return [
					{
						...
						type: 'toggle',
						value: currentSettings.ui.syntaxHighlight,
					...
					},
					{
						...
						label: 'Diff Colors',
						type: 'toggle',
						value: currentSettings.ui.diffColors,
						description: 'Show colored diffs'
					},
					{
						key: 'showLineNumbers',
						label: 'Line Numbers',
						type: 'toggle',
						value: currentSettings.ui.showLineNumbers,
					...
					},
					{
						...
						label: 'Owl Animation',
						type: 'category',
						description: currentSettings.ui.owlAnimation?.enabled
							? 'Enabled'
							: 'Disabled'
					},
					{ key: 'back', label: 'Back', type: 'action' }
			...
			case 'owl':
				const owlSettings = currentSettings.ui.owlAnimation || {
					enabled: false,
					flyWhenIdle: false,
					idleTimeout: 60000,
					sleepTimeout: 300000
				...
				return [
					{
						...
						label: 'Enable Owl',
						type: 'toggle',
						value: owlSettings.enabled,
						description: 'Show owl animation in dashboard'
					},
					{
						key: 'owlFly',
						label: 'Fly When Idle',
						type: 'toggle',
						value: owlSettings.flyWhenIdle,
						description: 'Owl flies around when idle'
					},
					{
						key: 'owlIdleTimeout',
						label: 'Idle Timeout',
						type: 'category',
						description: `${owlSettings.idleTimeout / 1000}s`
					},
					{
						key: 'owlSleepTimeout',
						label: 'Sleep Timeout',
						type: 'category',
						description: `${owlSettings.sleepTimeout / 1000}s`
					},
					{ key: 'back', label: 'Back', type: 'action' }
				];
			case 'permissions':
				return [
					{
						...
						label: 'View Allowed',
						type: 'category',
						description: `${currentSettings.permissions.allow.length} patterns`
					},
					{
						key: 'viewDeny',
						label: 'View Denied',
						type: 'category',
						description: `${currentSettings.permissions.deny.length} patterns`
					},
					{
						key: 'clearAllow',
						label: 'Clear Allowed',
						type: 'action',
						description: 'Reset allow list'
					},
					{
						key: 'clearDeny',
						label: 'Clear Denied',
						type: 'action',
						description: 'Reset deny list'
					},
					{ key: 'back', label: 'Back', type: 'action' }
				];
			case 'commands':
				...
				return [
					{
						...
						label: 'Execute Plan',
						type: 'action',
						description: 'Approve and execute the planned actions'
					},
					{
						key: 'plan-modify',
						label: 'Modify Plan',
						type: 'action',
						description: 'Request changes to the plan'
					},
					{
						key: 'plan-cancel',
						label: 'Cancel',
					...
					case 'backend':
						await saveAndUpdate({
							security: {
								...
								keyBackend: confirmation.payload
							}
						});
						break;
					case 'clear_allow':
						await saveAndUpdate({
							permissions: { ...currentSettings.permissions, allow: [] }
						});
						break;
					case 'clear_deny':
						await saveAndUpdate({
							permissions: { ...currentSettings.permissions, deny: [] }
						});
						break;
				}
			}
	...
	const handleSelect = useCallback(async () => {
		...
		if (item.type === 'category') {
			if (item.key === 'keyBackend') {
				...
				const currentIdx = backends.indexOf(
					currentSettings.security.keyBackend
				);
				...
				await saveAndUpdate({
					ui: {
						...currentSettings.ui,
						owlAnimation: { ...owlSettings, idleTimeout: nextTimeout }
					}
				});
				...
				await saveAndUpdate({
					ui: {
						...currentSettings.ui,
						owlAnimation: { ...owlSettings, sleepTimeout: nextTimeout }
					}
				});
			} else if (['provider'].includes(item.key)) {
				setView(item.key as MenuView);
			}
			...
			if (view === 'security') {
				...
				if (item.key === 'owlEnabled') {
					await saveAndUpdate({
						ui: {
							...currentSettings.ui,
							owlAnimation: { ...owlSettings, enabled: !item.value }
						}
					});
					...
					await saveAndUpdate({
						ui: {
							...currentSettings.ui,
							owlAnimation: { ...owlSettings, flyWhenIdle: !item.value }
						}
					});
				}
			}
			...
			} else if (view === 'provider' && item.key.startsWith('provider-')) {
				const providerMode = item.key.replace('provider-', '');
				await saveConfigUpdate({ provider: providerMode } as any);
	...
	if (confirmation) {
		return (
			...
			>
				...
				<Box marginBottom={1}>
					<Text color="white">{confirmation.message}</Text>
				</Box>
				<Box>
			...
			{view === 'permissions' && (
				<Box flexDirection="column" marginBottom={1} paddingX={0}>
					...
					{currentSettings.permissions.allow.length > 5 && (
						<Text color="gray" dimColor>
							{' '}
							... and {currentSettings.permissions.allow.length - 5} more
						</Text>
					)}
					...
					<Box marginTop={1}>
						...
						{currentSettings.permissions.deny.length > 5 && (
							<Text color="gray" dimColor>
								{' '}
								... and {currentSettings.permissions.deny.length - 5} more
							</Text>
						)}
					</Box>
				</Box>
