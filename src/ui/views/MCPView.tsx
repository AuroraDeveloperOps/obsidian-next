...
type ViewMode = 'list' | 'add_name' | 'add_command' | 'add_args' | 'certified' | 'edit_env';
...
export const MCPView: React.FC<MCPViewProps> = ({ onExit }) => {
        ...
        if (mode === 'list') {
            ...
            if (servers.length > 0) {
                ...
                if (input === 'e') {
                    ...
                    const keysToPrompt = regDef?.env ? Object.keys(regDef.env) : (selected.config.env ? Object.keys(selected.config.env) : []);

                    if (keysToPrompt.length > 0) {
                        startEnvSetup(selected.name, keysToPrompt);
                    } else {
                        setError(`Server '${selected.name}' has no configurable environment variables.`);
                    }
                }
            ...
            if (key.upArrow) {
                setCertifiedIndex(prev => (prev > 0 ? prev - 1 : registry.length - 1));
            }
            if (key.downArrow) {
                setCertifiedIndex(prev => (prev < registry.length - 1 ? prev + 1 : 0));
            }
            if (key.return) {
                ...
                try {
                    ...
                    await mcp.addServer(selected.name, {
    ...
    const startIdx = Math.max(0, Math.min(selectedIndex - Math.floor(VISIBLE_ITEMS / 2), servers.length - VISIBLE_ITEMS));
    ...
    return (
        <Box flexDirection="column" paddingX={1} borderStyle="round" borderColor="gray">
            <Box marginBottom={1} borderStyle="single" borderTop={false} borderLeft={false} borderRight={false} borderBottom={true} borderColor="gray">
                <Text bold color="red">MODEL CONTEXT PROTOCOL MANAGER</Text>
            </Box>

            {error && <Box marginBottom={1}><Text color="red">Error: {error}</Text></Box>}
            {statusMessage && <Box marginBottom={1}><Text color="green">{statusMessage}</Text></Box>}

            {mode === 'list' ? (
                <Box flexDirection="column">
                    <Box flexDirection="row" marginBottom={1}>
                        <Box width={30}><Text color="gray" bold>Server Name</Text></Box>
                        <Box width={15}><Text color="gray" bold>Status</Text></Box>
                        <Box flexGrow={1}><Text color="gray" bold>Command / Args</Text></Box>
                    </Box>

                    {servers.length === 0 ? (
                        <Box paddingY={2} justifyContent="center">
                            <Text color="gray">No servers configured. Press 'i' to install certified tools or 'a' to add a custom one.</Text>
                        </Box>
                    ) : (
                ...
                <Box flexDirection="column">
                    ...
                    <Box flexDirection="row" marginBottom={1}>
                        <Box width={20}><Text color="gray" bold>Tool</Text></Box>
                    ...
                    {registry.map((r, i) => {
                        ...
                        return (
                            <Box key={r.name} flexDirection="row">
                                <Box width={20}>
                                    <Text color={i === certifiedIndex ? 'yellow' : (isInstalled ? 'green' : 'white')} bold={i === certifiedIndex}>
                                        {i === certifiedIndex ? '> ' : '  '}
                                        {r.name}
                                    </Text>
                                </Box>
                    ...
                    {registry[certifiedIndex]?.requiresApiKey && (
                        <Box marginTop={1}>
                            <Text color="red">⚠️ Requires API Key/Token config in .obsidian/mcp.json</Text>
                        </Box>
                    )}
                </Box>
            ) : (
                <Box flexDirection="column" paddingY={1}>
                    ...
                    {mode === 'add_command' && (
                        <Box>
                            ...
                            <TextInput value={newCommand} onChange={setNewCommand} onSubmit={handleCommandSubmit} />
                        </Box>
                    )}
                    ...
                    {mode === 'add_args' && (
                        <Box>
                            ...
                            <TextInput value={newArgs} onChange={setNewArgs} onSubmit={handleArgsSubmit} />
                        </Box>
                    )}
            ...
            <Box marginTop={1} borderStyle="single" borderTop={true} borderLeft={false} borderRight={false} borderBottom={false} borderColor="gray">
                <Text color="gray">
                    {mode === 'list'
                        ? <Text><Text bold color="white">I</Text> Store · <Text bold color="white">A</Text> Add · <Text bold color="white">E</Text> Edit · <Text bold color="red">R</Text> Remove · <Text bold color="green">C</Text> Connect · <Text bold color="yellow">D</Text> Disconnect · <Text bold>Esc</Text> Back</Text>
                        : mode === 'certified'
                            ? <Text><Text bold color="white">Enter</Text> to install · <Text bold>Esc</Text> Cancel</Text>
                            : mode === 'edit_env'
                                ? <Text><Text bold color="white">Enter</Text> to Set · <Text bold>Esc</Text> Cancel</Text>
