...
export const Root = () => {
	...
	>();
	const [lastFileChange, setLastFileChange] = useState<{
		path: string;
		op: string;
	} | null>(null);
	...
	useEffect(() => {
		...
		const statHandler = (event: AgentEvent) => {
			// Update on completion events only - avoid re-renders during streaming
	...
	useEffect(() => {
		...
		// Throttle thought events to reduce UI spam
		if (event.type === 'thought') {
			const now = Date.now();
	...
	const handleExit = useCallback(async () => {
		...
		try {
			...
			bus.emitAgent({
				type: 'thought',
				content: `Session saved: ${sessionId}`
			});
		} catch (err) {
			bus.emitAgent({
				type: 'error',
							...
							<Box flexDirection="row" justifyContent="space-between">
								...
								<Box>
									{lastFileChange && (
										<Text dimColor>
											<Text
												color={lastFileChange.op === 'W' ? 'green' : 'yellow'}
											>
												[{lastFileChange.op}]
											</Text>{' '}
											{lastFileChange.path}
								...
								<Box>
									<Text dimColor>
										Context:{' '}
										{(usage.getContextUsage(stats.model).used / 1000).toFixed(
											1
										)}
										k /{' '}
										{(usage.getContextUsage(stats.model).limit / 1000).toFixed(
											0
										)}
										k (
										{Math.round(
											usage.getContextUsage(stats.model).percentRemaining ===
												100
												? 0
												: 100 -
														usage.getContextUsage(stats.model).percentRemaining
